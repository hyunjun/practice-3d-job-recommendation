...
test('search finds other locations and preserves their source through detail, saving, comparison and CSV', async ({ page }) => {
  ...
  expect(csv).toContain('"근무 국가","근무 국가 근거"')
  expect(csv).toContain('"확인 필요","Gurugram"')
...
test('region recovery discloses the broader scope, supports undo and keeps non-remote work filters in the same tab', async ({ page }) => {
  ...
  await expect(clearQuery.locator('dd span').first()).toHaveText('“Gurugram”')
  await expect(clearQuery.locator('dd strong')).toHaveText('검색어 지우기')
  await expect(clearQuery.getByRole('button', { name: '회사 1곳 · 공고 1개 보기', exact: true })).toBeVisible()
