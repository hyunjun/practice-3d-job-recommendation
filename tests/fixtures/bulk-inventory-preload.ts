...
const responsesFile = process.env.ORBIT_BULK_INVENTORY_RESPONSES
const requestLog = process.env.ORBIT_BULK_INVENTORY_REQUEST_LOG
const clockFile = process.env.ORBIT_BULK_INVENTORY_CLOCK
if (!responsesFile || !requestLog || !clockFile) throw new Error('Isolated bulk fixture paths are required')
Date.now = () => {
