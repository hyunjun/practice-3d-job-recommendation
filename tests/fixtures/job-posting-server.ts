...
import { JOB_POSTING_REGISTRATIONS, postingUpstreamResponses } from './job-posting'
...
export async function createJobPostingServer(directory: string, mode: Mode, companies: unknown[] = [...JOB_POSTING_REGISTRATIONS]) {
  ...
  if (mode === 'production') {
    await symlink(path.join(repository, 'dist'), path.join(cwd, 'dist'), 'dir')
  } else {
  ...
  await respond(postingUpstreamResponses())
  ...
  async function start() {
    ...
    const args = [
      ...
      '--import', pathToFileURL(path.join(repository, 'tests/fixtures/job-posting-preload.ts')).href,
      path.join(repository, mode === 'production' ? 'dist-server/index.mjs' : 'server/index.ts'),
    ...
    child = spawn(process.execPath, args, {
      cwd, env: {
        ...
        ORBIT_BOARDS_FILE: configFile, ORBIT_JOB_POSTING_REQUEST_LOG: requestLog,
        ORBIT_JOB_POSTING_RESPONSES_FILE: responsesFile,
      },
  ...
  async function stop(expectSuccess = true) {
    ...
    if (running.exitCode === null && running.signalCode === null) {
      ...
      expect(current.command).toContain('job-posting-preload.ts')
  ...
  async function verifyProductionBytes() {
    ...
    const expected = await readFile(path.join(repository, 'dist', entry!))
