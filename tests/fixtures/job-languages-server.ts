...
export async function createJobLanguagesServer(directory: string, mode: Mode, options: {
  buildRoot?: string
  port?: number
  companies?: unknown[]
} = {}) {
  const buildRoot = options.buildRoot ?? repository
  const companies = options.companies ?? [...LANGUAGE_REGISTRATIONS]
  ...
  await respond(languageUpstreamResponses())
  ...
  async function start() {
    ...
    const args = [
      ...
      '--import', pathToFileURL(path.join(repository, 'tests/fixtures/job-languages-preload.ts')).href,
    ...
    child = spawn(process.execPath, args, {
      cwd, env: {
        ...
        ORBIT_BOARDS_FILE: configFile, ORBIT_JOB_LANGUAGES_REQUEST_LOG: requestLog,
        ORBIT_JOB_LANGUAGES_RESPONSES_FILE: responsesFile,
      },
  ...
  async function stop(expectSuccess = true) {
    ...
    if (running.exitCode === null && running.signalCode === null) {
      ...
      expect(current.command).toContain('job-languages-preload.ts')
