...
import { PAGINATION_REGISTRATIONS, PAGINATION_TIME, paginationResponses } from './lever-pagination'
...
export async function createLeverPaginationServer(directory: string, mode: Mode, companies: unknown[] = [...PAGINATION_REGISTRATIONS], artifactRoot = repository) {
  ...
  async function start() {
    ...
    const args = [
      ...
      '--import', pathToFileURL(path.join(repository, 'tests/fixtures/lever-pagination-preload.ts')).href,
    ...
    child = spawn(process.execPath, args, {
      cwd, env: {
        ...
        ORBIT_BOARDS_FILE: configFile, ORBIT_LEVER_PAGINATION_REQUEST_LOG: requestLog,
        ORBIT_LEVER_PAGINATION_RESPONSES: responsesFile, ORBIT_LEVER_PAGINATION_CLOCK: clockFile,
      },
  ...
  async function stop(expectSuccess = true) {
    ...
    if (running.exitCode === null && running.signalCode === null) {
      ...
      expect(current.command).toContain('lever-pagination-preload.ts')
