...
import { BULK_TIME, bulkRegistrations, bulkResponses } from './bulk-inventory'
import type { BulkProvider } from './bulk-inventory'

const repository = fileURLToPath(new URL('../../', import.meta.url))
const exec = promisify(execFile)
type Mode = 'development' | 'production'
interface UpstreamRequest { at?: string; url: string; method: string; synthetic: boolean; networkSent: false; error?: string }
interface Run {
  pid: number
  cwd: string
  command: string[]
...
export async function createBulkInventoryServer(directory: string, mode: Mode, provider: BulkProvider, withControl = true, artifactRoot = repository) {
  ...
  if (mode === 'production') {
    await symlink(path.join(artifactRoot, 'dist'), path.join(cwd, 'dist'), 'dir')
  } else {
  ...
  await configure(bulkRegistrations(provider, withControl))
  ...
  await respond(bulkResponses(provider, 'healthy'))
  const clockFile = path.join(directory, 'clock.txt')
  const clock = (time: string) => writeFile(clockFile, time)
  await clock(BULK_TIME)
  const origin = `http://127.0.0.1:${port}`
  const runs: Run[] = []
  let child: ChildProcess | undefined
  let terminal: Promise<void> | undefined
  const saveReceipt = () => writeFile(path.join(directory, 'processes.json'), JSON.stringify({ mode, origin, runs }, null, 2))
  ...
  async function start() {
    ...
    const args = [
      ...
      '--import', pathToFileURL(path.join(repository, 'tests/fixtures/bulk-inventory-preload.ts')).href,
      mode === 'production' ? path.join(artifactRoot, 'dist-server/index.mjs') : path.join(repository, 'server/index.ts'),
    ...
    child = spawn(process.execPath, args, {
      cwd, env: {
        ...
        ORBIT_BOARDS_FILE: configFile, ORBIT_BULK_INVENTORY_REQUEST_LOG: requestLog,
        ORBIT_BULK_INVENTORY_RESPONSES: responsesFile, ORBIT_BULK_INVENTORY_CLOCK: clockFile,
      },
  ...
  async function stop(expectSuccess = true) {
    ...
    if (running.exitCode === null && running.signalCode === null) {
      ...
      expect(current.command).toContain('bulk-inventory-preload.ts')
  ...
  async function verifyProductionBytes() {
    ...
    const expected = await readFile(path.join(artifactRoot, 'dist', entry!))
