...
export async function createBoardConfigServer(directory: string, mode: Mode, companies: unknown[]) {
  ...
  async function start() {
    ...
    child = spawn(process.execPath, args, {
      cwd, env: {
        ...
        ORBIT_BOARDS_FILE: configFile, ORBIT_BOARD_TEST_REQUEST_LOG: requestLog,
      },
  ...
  async function stop(expectSuccess = true) {
    ...
    if (running.exitCode === null && running.signalCode === null) {
      ...
      expect(current.command).toContain('board-config-preload.ts')
  ...
  return {
    cwd, origin, configFile, defaultCache, defaultBytes, runs, start, stop, configure, requests, verifyProductionBytes,
    async cacheFiles() {
