...
export async function fetchGreenhouseBoard(company: Company, fetchedAt: string) {
  const payload = await fetchBoardJson(`https://boards-api.greenhouse.io/v1/boards/${encodeURIComponent(company.board!)}/jobs?content=true`, AbortSignal.timeout(BOARD_TIMEOUT)) as { jobs?: GreenhouseJob[] }
