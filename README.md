...
공개 데이터 화면에서 **관측 기록과 채용 분포**를 펼치면 날짜별 집계와 회사·근무 지역·개발 직무·근무 형태·기술의 분포를 확인할 수 있습니다. 이미 본문까지 수집한 자료를 사용하므로 기록을 읽거나 날짜·분포를 바꿀 때 외부 채용 사이트를 다시 조회하지 않습니다. 인재풀은 따로 세고, 미확인과 표본의 회사 편중·분모를 함께 표시합니다.

같은 회사·게시판과 분류 기준으로 정상 확인한 서로 다른 날짜의 기록이 2개 이상 있을 때만 공고 수의 차이를 비교합니다. 회사 추가나 분류 변경을 채용 증가·감소로 해석하지 않으며, 실패·미수집을 0개로 채우지 않습니다. 이전 캐시의 분류 기준이 없으면 분포만 표시하고 날짜 비교에서 제외합니다. [관측 방법·보존·해석 범위](docs/catalog-observations.md)를 참고하세요.
...
Coupang·Waymo·Snowflake·Docker·Palantir·ServiceNow 등을 포함한 공식 게시판 47곳에 이어 Workable 3곳과 Himalayas 7곳을 추가했습니다. 회사별 공식 커리어 주소·확인한 게시판·실제 수집 규모, 등록하지 않은 회사와 공개 잡 사이트의 조건은 [출처 조사 목록](docs/source-survey.md)에 정리했습니다. Google·Apple·Amazon·Meta 등의 공식 사이트도 조사했지만 아직 전체 목록·본문을 검증한 수집기를 연결하지 않았습니다.

Himalayas 공고에는 화면·상세·저장·가져오기 미리보기에서 제공자와 해당 사이트의 원문 링크를 표시합니다. CSV의 출처와 원문 URL, JSON의 `source`·URL에도 출처를 남깁니다. API 문서는 출처 표시와 링크를 전제로 앱·잡 보드의 사용을 허용하며, 다른 잡 애그리게이터로 재배포하는 것은 허용하지 않습니다. 이 앱은 외부 잡 사이트로 공고를 재전송하지 않으며, 실제 수집 자료도 Git에 포함하지 않습니다. [API 문서](https://himalayas.app/docs/remote-jobs-api)의 사용 조건을 참고하세요.
...
- Workable은 문서화된 공개 위젯 API의 전체 목록과 회사 이름·공고 shortcode·원문 URL을 검증합니다. 본문 수집은 `details=true`, 목록 확인은 본문 없는 응답을 사용합니다. `hidden: true` 위치는 표시하거나 기본 주소로 복구하지 않으며, `telecommuting: false`만으로 출근·하이브리드를 확정하지 않습니다.
...
관측 집계는 `.local/observations-v1/<본문 캐시 파일명>`에 저장합니다. 최근 90개 UTC 날짜 범위에서 최대 4개의 회사 구성·분류 기준을 보존하며, 파일은 최대 16 MiB입니다. 손상된 기록을 빈 파일로 덮어쓰지 않고 오류를 안내합니다. 매일 자동 수집하는 스케줄러가 추가된 것은 아니며, 조회한 날의 기록부터 쌓입니다.
...
직군 분류는 버전 5를 사용하며 이전 버전 1·2·3·4도 읽습니다. 기존 API 응답·캐시·저장 공고·JSON 백업에 같은 기준을 적용합니다. 저장소와 백업 파일의 바깥 형식은 유지하며, 공고의 분류를 다시 해석한 것만으로 게시 내용이 바뀌었다고 표시하지 않습니다.

`Finance Lead, Product & Engineering`처럼 재무·회계 직무가 지원하는 부서를 제목 뒤에 적은 경우 개발직으로 포함하지 않습니다. 금융 부서의 소프트웨어 엔지니어·데이터 과학자와 명시적인 컴퓨팅 공동 역할은 유지하며, 공개된 관리직 정보가 있으면 우선 적용합니다.
