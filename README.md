...
- **저장:** 최대 500개 공고의 스냅샷, 메모, 지원 완료 표시, 삭제 실행 취소, CSV 내보내기. 공고별로 비동기 저장하며, 저장 중·완료·실패와 재시도를 표시합니다. 새로고침 후에도 원래 출처와 보상 조건이 유지됩니다.
...
이전 저장 목록은 자동으로 옮깁니다. 일부 항목의 형식이 잘못되어도 정상 기록을 복구하고, 읽지 못한 항목·중복·한도 초과 등이 있으면 이전 원본 전체를 별도로 보관합니다. 원본은 **따로 보관한 원본이 있어요 → 보관한 원본 내려받기**에서 확인할 수 있습니다. 원본에는 정상 기록의 메모도 포함될 수 있으며, 목록에서 개별 공고를 삭제해도 이 원본은 남습니다.

저장 실패 시 마지막 입력은 현재 탭에 유지됩니다. **현재 내용 CSV로 보관**하거나 불필요한 기록을 정리한 뒤 **저장 다시 시도**를 선택할 수 있습니다. 처음부터 저장소를 읽지 못하면 기존 기록을 빈 목록으로 덮어쓰지 않고 새 저장을 잠시 막으며, 기회 탐색은 계속 사용할 수 있습니다.

같은 주소의 다른 탭에 저장 완료를 알리고 변경을 다시 읽습니다. 서로 다른 항목의 수정은 합치지만, 같은 메모를 동시에 수정하면 마지막에 완료된 값이 남습니다. 저장 데이터는 서버로 전송하지 않습니다. 브라우저의 사이트 데이터 삭제·저장 공간 회수·비공개 창 종료로 기록이 없어질 수 있으므로 필요한 기록은 CSV로 보관하세요. CSV와 원본 파일을 앱으로 가져오는 기능은 아직 없습니다. [저장소 구조·이전·실패 처리](docs/saved-storage.md)에 구체적인 동작과 제한을 정리했습니다.
