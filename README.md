...
- **직무 분류와 근거:** 공고 제목과 공개 부서·팀의 표기를 확인합니다. 여러 직무가 명시되면 각각의 필터에서 찾을 수 있고, 세부 직무를 특정하지 못한 공고도 따로 탐색합니다.
...
저장 목록의 **게시 상태 확인**은 탐색 모드와 조건을 바꾸지 않습니다. 저장한 공고의 ID·메모·프로필을 서버에 보내지 않고, 전체 대상 게시판의 공개 목록을 받아 브라우저에서 비교합니다. 포지션·직무 분류와 근거·근무지·근무/고용/비자/취업 자격·보상·기술/경력·본문·지원 링크의 표시 내용이 달라졌는지 확인하며, 저장한 기록은 자동으로 덮어쓰지 않습니다. CSV에는 지원 상태와 공개 게시 상태, 확인 시각, 내용 비교 결과를 별도로 남깁니다.
...
- 여러 직무가 명시된 공고는 각 직무의 필터와 희망 직무 비교에 포함합니다. 한 공고나 회사를 중복 집계하지 않으며, 분류에 사용한 제목·부서 원문을 상세·저장·CSV에서 확인할 수 있습니다.
...
src/
  ...
  components/JobRoleDetails.tsx  세부 직무와 제목·부서의 분류 근거
...
이전 공개 공고의 직무도 저장된 제목으로 다시 분류합니다. 보관하지 않은 부서 정보는 추정하지 않고 다음 정상 조회에서 확인하며, 원래 조회 시각·메모·지원 기록은 유지합니다. 샘플 공고의 명시적인 직무 설정은 유지합니다. `Job.roleClassification.roles`가 전체 직무 목록이며, `Job.role`은 기존 형식과 호환하는 대표값 또는 `unknown`입니다. 필터·매칭·표시에서는 `shared/job-roles.ts`의 공통 함수를 사용합니다.

직무 분류는 게시된 표기를 이용한 탐색 보조입니다. 제목에서 제품 분야를 언급한 것과 실제 담당 업무가 일치하지 않을 수 있고, 부서가 해당 직무보다 넓은 범위일 수 있습니다. 개발 직군을 수집하는 제목 규칙과 세부 직무 분류는 별개이며, 모든 직업·직급을 검증한 분류 체계는 아닙니다.
...
`/api/posting-status`는 등록된 게시판의 전체 공개 ID, 탐색 대상 공고의 표시 항목별 SHA-256 값, 회사별 확인·실패·재시도 시각을 반환합니다. `?refresh=1`은 기존 회사별 재조회 정책을 따르며 카탈로그와 진행 중인 수집을 공유합니다. 저장한 공고 ID나 URL을 입력받는 API가 아니며 임의 사이트를 조회하지 않습니다. 전체 공개 ID는 개발 직무 선별 전에 확보하고, 완전한 조회에 성공했을 때만 회사별 캐시에 반영합니다. 지도에 연결되지 않은 개발 공고의 내용도 비교 대상에 포함합니다.
