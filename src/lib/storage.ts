...
import { REMOTE_SCOPE_CAUTION, remoteScopeLabel, remoteScopeNote } from '../../shared/job-remote'
...
export function loadExploration(profile: Profile): ExplorationState {
  ...
  const schema = z.object({
    ...
    filters: z.object({
      ...
      postingType: z.enum(['opening', 'talent-pool', 'all']).catch(filters.postingType),
...
export function exportSavedCsv(saved: SavedJob[], observations?: ReadonlyMap<string, PostingObservation>): void {
  ...
  const rows = [
    ['회사', '포지션', '근무지', '데이터', '상태', '저장일', '메모', '채용 링크', '연봉', '보상 조건', '보상 근거', '기술 조건', '경력 조건', '기술·경력 근거', '공개 게시 상태', '게시 목록 확인 시각', '내용 비교', '저장 내용과 다른 항목', '비자 지원', '취업 자격 조건', '취업 자격 근거', '직무 분류', '직무 분류 근거', '탐색 직군', '탐색 직군 근거', '저장 내용의 조회 시각', '내보낼 때의 조회 기록', '내보낸 시각', '근무지 판단', '원래 게시 위치', '본문의 근무지', '근무지 원문 근거', '명시된 원격근무 국가·지역', '원격근무 국가 코드', '원격근무 추가 확인', '고용 형태', '고용 형태 근거', '원격근무 지역 판단', '원격근무 지역 원문 근거', '모집 유형', '모집 유형 근거', '언어 조건', '언어 조건 근거', '시간대·협업 시간', '시간대·협업 시간 근거', '근무 국가', '근무 국가 근거', '공고 내용 확인 시각'],
    ...current.map(item => [
      item.company.name, item.job.title, item.job.locationLabel,
      item.job.source === 'himalayas' ? 'Himalayas · https://himalayas.app' : JOB_SOURCE_LABELS[item.job.source],
      ...
      item.job.locationResolution ? item.job.locationResolution.status === 'relocation' ? '제목·본문의 근무지로 표시' : '정보 불일치 · 근무지 확인 필요' : '',
      ...
      EMPLOYMENT_LABELS[item.job.employment],
      item.job.evidence?.employment ? `${item.job.evidence.employment.source === 'title' ? '공고 제목' : item.job.evidence.employment.source === 'board' ? '공개 게시판 정보' : '공고 본문'}\n${item.job.evidence.employment.text}` : '',
      remoteScopeNote(item.job),
      item.job.remoteScopeResolution?.evidence.map(evidence => evidence.text).join('\n\n') ?? '',
      POSTING_TYPE_LABELS[isTalentPoolJob(item.job) ? 'talent-pool' : 'opening'],
      ...
      item.job.workMode === 'remote' ? '' : item.workplace.countries.map(countryName).join(' · ') || '확인 필요',
      item.job.workMode === 'remote' ? '' : item.workplace.evidence.map(evidence => evidence.text).join('\n\n'),
      observations?.get(item.job.id)?.contentCheckedAt
        ?? (observations?.get(item.job.id)?.changedFields ? observations.get(item.job.id)?.checkedAt ?? '' : ''),
    ]),
  ]
