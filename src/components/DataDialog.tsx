  ...
  return <Dialog title="기회의 지도, 그 안의 데이터." eyebrow="TRANSPARENT BY DESIGN" onClose={onClose} className="data-dialog">
    <div className="dialog-body">
      <div className="data-source-options">
        <button disabled={loading} className={isSample ? 'selected' : ''} aria-pressed={isSample} onClick={() => onSource('sample')}><span className="data-option-icon"><Globe2 size={21} /></span><strong>샘플로 탐색</strong><span>설정 없이 전체 경험을 체험해요</span><small>가상의 공고 · 실제 회사 채용 페이지</small>{isSample && <CheckCircle2 size={16} className="data-option-check" />}</button>
      ...
      <div className="coverage-stats"><div><strong>{catalog.cities.length}</strong><span>제공 도시</span></div><div><strong>{catalog.companies.length}</strong><span>대상 회사</span></div><div><strong>{catalog.jobs.length.toLocaleString()}</strong><span>{isSample ? '샘플 공고' : '조회된 개발 공고'}</span></div></div>
      ...
      <section className="data-explanation"><h3><Globe2 size={16} />현재 제공하는 지역</h3><div className="coverage-city-list">{catalog.cities.map(city => <span key={city.id}>{city.name}</span>)}</div><p>{isSample ? '샘플의 채용 여부, 보상, 비자 조건은 모두 체험을 위한 예시입니다. 실제 지원 전 회사 채용 페이지를 확인해 주세요.' : `${catalog.unmappedCount}개 개발 공고는 근무지가 제공 범위 밖이거나 위치를 확정할 수 없어 지도에 포함하지 않았어요.`}</p></section>
      ...
      <section className="data-explanation"><h3><ShieldCheck size={16} />추천과 개인정보</h3><p>기술 키워드, 희망 직무, 경력 연수를 기준으로 공고를 정렬해요. 비자·근무 형태·연봉은 확인된 정보와 미확인을 구분합니다. 보상 비교에는 고정 참고 환율을 사용하며, 지역별 생활비나 합격 확률을 추정하지 않아요.</p><p>이력서 파일과 원문은 서버나 외부 AI로 전송하지 않아요. 기억하기를 선택하면 확인한 프로필만 이 브라우저에 저장됩니다. 프로필 화면에서 언제든 삭제할 수 있어요.</p></section>
    </div>
  </Dialog>
}
