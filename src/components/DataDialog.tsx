  ...
  return <Dialog title="기회의 지도, 그 안의 데이터." eyebrow="TRANSPARENT BY DESIGN" onClose={onClose} className="data-dialog">
    <div className="dialog-body">
      <div className="data-source-options">
        ...
        <button disabled={loading || retryIn > 0} className={!isSample ? 'selected' : ''} aria-pressed={!isSample} onClick={() => onSource('greenhouse')}><span className="data-option-icon"><Database size={21} /></span><strong>공개 채용공고</strong><span>Greenhouse 게시판에서 직접 조회해요</span><small>API 키 없이 · 인터넷 연결 필요</small>{!isSample && <CheckCircle2 size={16} className="data-option-check" />}</button>
      </div>
      ...
      <div className="board-list">{catalog.boards.map(board => {
        ...
        return <div className="board-row" key={`${board.companyId}-${board.board}`}>
          ...
          <div className="board-copy">
            <div className="board-name"><strong>{company.name}</strong><span className={board.status === 'ok' && !retained ? 'board-ok' : 'board-error'}>{retained ? `이전 ${board.included}개 유지` : board.status === 'ok' ? `${board.included}개 반영` : '데이터 미확인'}</span></div>
