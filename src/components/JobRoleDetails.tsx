...
export function JobRoleDetails({ job }: { job: Job }) {
  ...
  const classification = current.roleClassification!
  ...
  return <><JobOccupationNotice job={current} /><section className="job-role-details" aria-label="직무 분류">
    ...
    <p>{classification.roles.length
      ? classification.evidence.some(evidence => evidence.source === 'description')
    ...
    {(research || !isTechnicalJob(current)) && <details className="qualification-evidence occupation-evidence">
