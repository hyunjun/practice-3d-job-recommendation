      ...
      <section className="profile-form">
        {step === 1 ? <>
          ...
          <div className="form-grid preference-grid">
            ...
            <div className="field-group"><label htmlFor="profile-country">원격근무 시 거주 국가·지역</label><select id="profile-country" value={draft.residence} onChange={event => setDraft({ ...draft, residence: event.target.value })}>{COUNTRIES.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select><p className="field-description">지도에 표시되지 않는 국가·지역도 선택할 수 있어요.</p></div>
          ...
          <p className="field-description">거주 국가는 원격근무 지역 비교에만 사용해요. 국적이나 취업 허가 보유 여부로 간주하지 않습니다. 비자 지원에 국가별 제한이 있으면 조건부 지원으로 표시해요.</p>
