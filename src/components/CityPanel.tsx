...
export function CityPanel(props: Props) {
  ...
  return <aside className="results-panel" aria-label="도시와 회사 탐색 결과">
    ...
    <button className="panel-data-footer" onClick={onData}><span className={`source-status-dot ${catalog.source === 'sample' ? 'sample' : ''}`} /><span>{catalog.source === 'sample' ? '샘플 데이터로 탐색 중' : !catalog.fetchedAt ? '공개 공고 연결 확인' : catalog.stale ? '이전 공개 공고 · 연결 확인 필요' : 'Greenhouse 공개 채용공고'}</span><CircleHelp size={14} /></button>
  </aside>
}
