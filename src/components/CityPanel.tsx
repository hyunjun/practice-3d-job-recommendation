...
export function CityPanel(props: Props) {
  const { catalog, results, remote, selectedId, tab, profile, compareIds, savedIds, onTab, onSelect, onHover, onCompare, onOpenJob, onSave, onProfile, onData, onFilters, onReset } = props
  ...
  return <aside className="results-panel" aria-label="도시와 회사 탐색 결과">
    <div className="results-tabs">
      <button className={tab === 'cities' ? 'active' : ''} aria-pressed={tab === 'cities'} onClick={() => onTab('cities')}><MapPin size={15} />도시 탐색<span>{results.length}</span></button>
      <button className={tab === 'remote' ? 'active' : ''} aria-pressed={tab === 'remote'} onClick={() => onTab('remote')}><Globe2 size={15} />원격 기회<span>{remoteCompanies.length}</span></button>
    </div>
    <div className="results-scroll">
      {tab === 'cities' ? selectedCity ? <>
    ...
    <button className="panel-data-footer" onClick={onData}><span className={`source-status-dot ${catalog.source === 'sample' ? 'sample' : ''}`} /><span>{catalog.source === 'sample' ? '샘플 데이터로 탐색 중' : catalog.stale ? '이전 공개 공고 · 연결 확인 필요' : 'Greenhouse 공개 채용공고'}</span><CircleHelp size={14} /></button>
  </aside>
}
