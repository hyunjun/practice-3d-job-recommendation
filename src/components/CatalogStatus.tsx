...
export function CatalogStatus({ catalog, loading, progress, error, expired, retryAt, onRetry, onData }: Props) {
  const retryIn = useRetryCountdown(!catalog.fetchedAt || error || catalogNeedsAttention(catalog) ? retryAt : undefined)
  if (loading && progress) return <section className="catalog-notice catalog-collecting">
    <CollectionProgress catalog={catalog} progress={progress} />
    <button className="text-button" onClick={onData}>회사별 조회 상태</button>
  </section>
  ...
  return <div className="catalog-notice" role="status">
    ...
    <p>{loading ? '공개 공고를 조회하고 있어요.' : error || (ageOnly ? '공고를 다시 확인할 시간이 됐어요.' : '일부 게시판의 최신 공고를 확인하지 못했어요.')}<span>{catalog.source === 'sample' ? '현재는 샘플 결과를 표시하고 있어요.' : error || loading ? '현재까지 확인한 공고를 유지합니다. 조회 시각은 공고에서 확인할 수 있어요.' : `이전 조회 공고 ${health.retained}개를 포함합니다.${health.unavailable ? ` ${health.unavailable}개 회사는 확인 가능한 공고가 없어요.` : ''}`}</span></p>
