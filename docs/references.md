...
지도 표시 범위와 공고 수집 범위는 구분합니다. 초기 구현에서는 기타 근무지를 전 세계 범위에서만 표시하고, 지역을 해제하는 변경안에는 범위가 넓어진다는 설명을 붙였습니다. 이후 근무 국가를 확인할 수 있는 공고는 해당 지역에서도 찾도록 보완했습니다. 기존 캐시에 개수만 남아 있는 공고는 실제 열람 가능한 목록으로 계산하지 않으며 다음 정상 조회에서 갱신합니다. 이 화면 구성은 자료의 구분과 실제 응답을 바탕으로 정한 제품 설계이며, 제공자가 요구하는 특정 UI 규칙은 아닙니다.
...
## 조회 대기 중에도 읽을 수 있는 모드 선택

| 참고 자료 | 확인한 내용 | 반영 |
|---|---|---|
| [WAI-ARIA APG: Button Pattern](https://www.w3.org/WAI/ARIA/apg/patterns/button/) | 토글 버튼의 이름은 선택 여부에 따라 바꾸지 않고 `aria-pressed`로 상태를 전달 | 기존 버튼 이름과 선택 상태를 유지하고, 시각적인 ‘선택됨’ 배지는 접근 가능한 이름에서 제외 |
| [WCAG 2.2: Contrast (Minimum)](https://www.w3.org/WAI/WCAG22/Understanding/contrast-minimum.html) | 일반 텍스트의 최소 명암비는 4.5:1이며 비활성 컨트롤은 예외 | 조회 대기로 비활성화된 경우에도 현재 모드의 제목과 배지에 4.5:1을 앱의 가독성 목표로 적용 |
| [WCAG 2.2: Use of Color](https://www.w3.org/WAI/WCAG22/Understanding/use-of-color.html) | 상태를 색상만으로 전달하지 않고 글자·모양 등 함께 제공 | 기존 체크 모양에 ‘선택됨’ 문구를 더해 샘플·공개 중 현재 모드를 명시 |

2026-09-26에 공식 문서를 확인했습니다. 기존 화면에도 체크 모양과 `aria-pressed`가 있었습니다. 이번 변경은 선택 표시를 더 분명하게 읽도록 하는 개선이며, 기존 비활성 카드가 WCAG 명암비 기준을 위반했다는 판단은 아닙니다. 조회 중·재시도 대기 중의 버튼 비활성화와 요청 제한은 유지합니다.

## 저장 기록이 많을 때의 페이지 탐색

| 참고 자료 | 확인한 내용 | 반영 |
|---|---|---|
| [GOV.UK Design System: Pagination](https://design-system.service.gov.uk/components/pagination/) | 긴 목록을 나눌 때는 성능·사용성의 실제 개선을 확인하고, 한 페이지뿐이면 페이지 이동을 표시하지 않음 | 저장 목록을 12개씩 표시하고 전체 결과 수·현재 범위·이동 경계를 안내. 긴 목록의 위·아래에 이동 버튼 제공 |
| [web.dev: How large DOM sizes affect interactivity](https://web.dev/articles/dom-size-and-interactivity) | 큰 DOM은 초기 렌더링과 이후 상호작용의 스타일·레이아웃 작업을 늘릴 수 있음 | 현재 페이지의 카드와 추천 설명만 렌더링하고, 검색·집계·내보내기는 전체 저장 기록을 사용 |
| [WCAG 2.2: Focus Order](https://www.w3.org/WAI/WCAG22/Understanding/focus-order.html) | 키보드 초점의 순서는 의미와 조작 흐름을 유지해야 함 | 페이지 이동 후 새 목록의 첫 공고에 초점을 두고 화면에 표시. 검색·필터 변경은 조작 중인 입력의 초점을 유지 |

2026-09-26에 공식 문서를 확인했습니다. 12개는 세 열·두 열·한 열 배치에 맞춘 앱의 선택이며 위 문서가 정한 개수는 아닙니다. 페이지 이동에는 앱 내부 상태를 바꾸는 버튼을 사용합니다. 로컬에서 500개 가상 저장 기록으로 전후 DOM 크기와 입력 반영 시간을 측정했으며, 이 측정은 실제 구직 기록이나 현장 INP를 대신하지 않습니다.

## 상세 창을 닫은 뒤 키보드 탐색 이어가기

| 참고 자료 | 확인한 내용 | 반영 |
|---|---|---|
| [WAI-ARIA APG: Dialog (Modal) Pattern](https://www.w3.org/WAI/ARIA/apg/patterns/dialog-modal/) | 대화상자를 닫으면 원래 실행한 요소로 초점을 돌려주며, 그 요소가 사라졌다면 작업 흐름을 이어갈 수 있는 요소를 선택 | 원래 버튼을 우선하고, 저장 목록에서 사라진 공고는 현재 페이지의 다음·이전 공고로 연결. 결과가 비었을 때는 검색 또는 탐색으로 연결 |
| [WCAG 2.2: Focus Not Obscured (Minimum)](https://www.w3.org/WAI/WCAG22/Understanding/focus-not-obscured-minimum.html) | 고정된 알림·머리글·바닥글이 키보드 초점을 가릴 수 있으며, `scroll-padding`을 대응 기법으로 설명 | 실제 알림 높이와 하단 간격에 맞춰 스크롤 여유를 확보하고, 알림이 겹치는 컨트롤을 화면에 표시 |

2026-09-26에 공식 문서를 확인했습니다. 남은 공고의 순서와 빈 목록의 복귀 위치는 위 원칙에 따라 앱의 작업 흐름에 맞춰 정했습니다. 상세를 열 당시의 검색 순서를 기준으로 현재 화면에 남은 공고를 찾고, 같은 공고의 버튼이 다시 만들어졌으면 그 공고로 돌아갑니다. 새 대화상자가 열린 경우에는 해당 창의 초점을 유지합니다.

알림 높이는 문구의 줄바꿈과 화면 크기에 따라 다시 측정합니다. 알림이 표시되는 동안 페이지 끝에서도 컨트롤을 드러낼 수 있는 공간을 두고, 키보드 초점이 알림과 겹치면 보이는 위치로 스크롤합니다.

## 지도에 없는 근무지의 국가와 지역

| 참고 자료 | 확인한 내용 | 반영 |
|---|---|---|
| [Google: JobPosting의 jobLocation](https://developers.google.com/search/docs/appearance/structured-data/job-posting#joblocation) | 실제 일을 수행할 장소와 국가를 표시하며 원격근무·지원자 거주 범위는 별도 속성 | 물리적 근무 국가를 지역 검색에 사용하고, 도시 좌표나 원격근무 자격으로 바꾸지 않음 |
| [Ashby Public Job Posting API](https://developers.ashbyhq.com/docs/public-job-posting-api) | 주·추가 근무지의 주소와 `addressCountry`를 제공 | 각 근무지의 국가 필드를 해당 위치와 함께 보존 |
| [SmartRecruiters Location](https://developers.smartrecruiters.com/docs/location) | `country`는 ISO 두 글자 국가 코드이고 `region`·`city`·`remote`는 별도 필드 | 국가와 도시를 독립적으로 읽고, 미지원 도시의 국가도 유지 |
| [Lever Postings API](https://github.com/lever/postings-api) | `location`은 주 근무지이고 `allLocations`에도 포함되며, `country`는 국가·지역 코드 또는 미확인의 `null` | 주 근무지의 국가를 다른 근무지에 복사하지 않고 보존. 주 위치 이름이 없으면 국가 근거를 별도로 유지 |
| [UN M49](https://unstats.un.org/unsd/methodology/m49/overview/) | 국가·지역 코드와 통계용 지리 구분. Estonia는 Northern Europe, Malaysia는 South-eastern Asia에 포함 | 기존 국가 목록의 지역 구분을 기타 근무지 검색에도 사용 |

2026-09-26에 공식 문서를 확인했습니다. 같은 날 기본 24개 게시판의 새 조회에서 개발·연구 모집 항목 1,317개와 지도에 연결되지 않은 항목 268개를 확인했습니다. 후자에는 `Tallinn, Estonia` 계열 21개와 `Petaling Jaya, Malaysia` 47개처럼 국가를 확인할 수 있는 위치가 있었습니다. 수치는 해당 조회 범위의 관측이며 전체 채용 시장의 규모가 아닙니다.
...
## 공개 공고의 주직무와 부서·제품 이름

2026-09-26 수집한 자료에서 OpenAI·Discord의 디자인 직무, Roblox의 커뮤니티 담당 직무, Pinterest의 행정 지원 직무를 확인했습니다. 제목의 `Engineering`·`Developer`는 지원 대상 부서나 이용자를 뜻하기도 했습니다. SpaceX·OpenAI의 일부 공력·추진·회로·액추에이터 직무도 소프트웨어 개발과 구분할 필요가 있었습니다.

분류 보완은 제목에 명시된 주직무에 적용합니다. 개발 도구를 만드는 EDA 직무의 `RFIC Engineering` 부서나 소프트웨어 인프라 직무의 `Flight` 분야를 이유로 개발직을 제외하지 않습니다. 소프트웨어·펌웨어·임베디드·AI·데이터·컴퓨팅 연구를 명시한 직무를 보존하고, 물리 분야 업무에서 Python이나 시뮬레이션 도구를 사용한다는 사실만으로 개발직이라고 단정하지 않습니다.

별도 검증 담당은 원문 업무를 읽고 확실한 제외 사례와 개발직 대조군을 골랐습니다. 원문·수집 시각·파일 해시와 모호한 사례의 검토 기록은 `.local/research/58/independent/`에 보관합니다. 공개 회귀 검사는 가상 회사와 직접 작성한 본문을 사용합니다.

앞서 참고한 O*NET의 개발·연구·지원 직군 구분을 바탕으로 앱의 탐색 범위를 보완한 것입니다. 모든 `Engineer`·`Developer` 직함을 새로 검증하는 규칙이나 공식 직업 코드 판정은 아닙니다. Developer Advocate·Physical Design 등 여러 성격의 업무가 섞인 직무와 아직 구분하지 못한 전문 분야는 추가 검토가 필요합니다.

## 주요 IT 기업과 공개 잡 사이트의 추가 조사

2026-09-26 UTC에 기존 공개 목록과 별도로 기업 후보 107곳과 공개 잡 사이트 12곳을 확인했습니다. 공식 커리어 페이지, 연결된 게시판, 제공자 API의 회사 표시·공고 ID·본문을 대조해 47개 게시판을 추가했습니다. [전체 조사 목록과 회사별 근거](source-survey.md)에 공식 URL, 게시판 식별자, 실제 수집 수치와 미등록 사유를 정리했습니다.

회사 이름과 게시판 식별자가 다른 DigitalOcean(`digitalocean98`), Automattic(`automatticcareers`), Miro(`realtimeboardglobal`)는 해당 제공자의 공개 회사 표시를 확인했습니다. Snowflake·ClickHouse·Plaid·Confluent의 Ashby 게시판도 회사 표시와 공고를 확인했습니다. 일부 이전 Greenhouse·Lever 후보 주소는 404를 반환했으며 정상 빈 목록으로 기록하지 않았습니다.
